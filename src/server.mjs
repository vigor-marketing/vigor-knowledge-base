...
app.get('/api/v1/search', async (request, reply) => {
  ...
  const rawResults = semantic ? (await searchIndex.hybridSearch({ query, vector: (await embeddings.embed([query]))[0], securityLevels })).map(result => ({ chunkId: result.chunkId, documentId: result.documentId, versionId: result.versionId, content: result.content, headingPath: result.headingPath, score: result.score })) : await repository.keywordSearch(query, 2, securityLevels)
