...
| `ELASTICSEARCH_URL` / `ELASTICSEARCH_API_KEY` | 混合检索索引 |
...
3. 用 `docker compose -f docker-compose.selfhosted.yml --env-file .env.selfhosted up -d --build` 启动专属 MySQL、Redis、Elasticsearch、知识库 API 和 Worker；除 API 的 4180 测试端口外，不公开任何数据服务端口。
...
6. 发布文档，确认 Elasticsearch Chunk 索引、关键词搜索、向量搜索、引用来源和 DeepSeek 回答。
