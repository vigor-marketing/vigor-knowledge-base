...
function renderManagementRecords() {
  ...
  list.innerHTML = state.manageable.length ? state.manageable.map(document => `<article class="manage-row record ${document.status === 'archived' ? 'archived-record' : ''}"><div><b>${escapeHtml(document.title)}</b><p class="hint ${document.status === 'archived' ? 'record-status' : ''}">${escapeHtml(document.versionLabel)} · ${document.status === 'archived' ? '已归档（不可下载）' : escapeHtml(document.status)} · 更新于 ${formatDate(document.updatedAt)}</p></div><div class="record-actions">${document.status === 'draft' ? `<button class="publish-document" data-document-id="${escapeHtml(document.documentId)}" type="button">发布到下载</button>` : ''}${document.status === 'archived' ? `<button class="restore-document" data-document-id="${escapeHtml(document.documentId)}" type="button">重新启用</button>` : `<button class="update-document" data-document-id="${escapeHtml(document.documentId)}" type="button">更新版本</button><button class="archive-document" data-document-id="${escapeHtml(document.documentId)}" type="button">归档</button>`}</div></article>`).join('') : '<p class="hint">暂无资料记录。</p>'
